 ...
 */
export function drawLabScatterPlotRevised (canvas, pixelData, imageWidth, imageHeight, pixelSampleFactor = 1) { // Export the function
  const ctx = canvas.getContext('2d');
  const canvasWidth = canvas.width;
  const canvasHeight = canvas.height;
  ...
  ctx.textAlign = 'left';
  ctx.fillText('+a*', canvasWidth - 80, canvasCenterY + 15);
  ctx.textAlign = 'right';
  ctx.fillText('-a*', 80, canvasCenterY + 15);
  ctx.textAlign = 'center';
  ...
  const plotRangeA = 240; // From -120 to +120
  const plotMinA = -120;
  const plotRangeB = 240; // From -120 to +120
  const plotMinB = -120;
  ...
  const pointSize = 2.5; // Increased point size for better visibility
  ...
  for (let i = 0; i < totalPixels; i += pixelSampleFactor) {
    ...
    const x = canvasCenterX + a_lab * scaleX;
    ...
    const y = canvasCenterY - b_lab * scaleY;
